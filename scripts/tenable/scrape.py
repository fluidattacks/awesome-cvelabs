#!/usr/bin/env python3
"""Tenable Research scraper — Playwright edition.
Source: Sitemap → research/CVE blog posts → CVE IDs from URL slugs.
"""
...
async def _get_advisory_urls(api) -> list[str]:
    ...
    for page_num in [1, 2]:
        try:
            resp = await api.get(f"{SITEMAP_BASE}?page={page_num}")
            if resp.status != 200:
                continue
    ...
    try:
        resp = await api.get(SITEMAP_BASE)
        if resp.status == 200:
            content = await resp.text()
            locs = re.findall(r"<loc>([^<]+)</loc>", content)
